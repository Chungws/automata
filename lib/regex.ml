...
let has_backref pattern =
  let rec check i =
    ...
    else if pattern.[i] = '\\' && pattern.[i + 1] >= '1' && pattern.[i + 1] <= '9' then true
    else check (i + 1)
  in
  check 0

(* 엔진 자동 선택: 백레퍼런스 있으면 Backtrack, 없으면 NFA *)
let auto_select_engine pattern =
  if has_backref pattern then Backtrack else NFA
...
let match_groups pattern text =
  Regex_bt.match_groups pattern text
