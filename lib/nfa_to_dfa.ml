...
let compute_reachable nfa =
  let start = Nfa.epsilon_closure nfa (Nfa.StateSet.singleton nfa.start) in
  let rec loop visited result queue =
    ...
    | cur :: rest ->
        ...
        if StringSet.mem cur_name visited then loop visited result rest
          ...
          let nexts =
            List.map (fun ch -> Nfa.step nfa cur (Some ch)) nfa.alphabet
          in
...
let build_transitions nfa reachable =
  ...
  |> Seq.fold_left
       (fun trans (state_set, ch) ->
         let next = Nfa.step nfa state_set (Some ch) in
